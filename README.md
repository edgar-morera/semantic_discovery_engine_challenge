...
Una vez hecho el setup inicial, hay que setear la variable de entorno `HUGGINGFACE_API_KEY` con un token válido de HuggingFace y ejecutar `make restart` para recrear los contenedores.
...
Con el entorno levantado, este es el flujo mínimo para crear un producto, indexarlo y hacer una búsqueda semántica (ver la lista completa de endpoints en [API](#api)):
